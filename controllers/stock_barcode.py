...
class StockBarcodeController(http.Controller):
    ...
    def get_barcode_data(self, model, res_id):
        ...
        mute_sound = request.env['ir.config_parameter'].sudo().get_param('mhma_stock_barcode.mute_sound_notifications')
    ...
    def rid_of_message_demo_barcodes(self, **kw):
        ...
        action = request.env.ref('mhma_stock_barcode.stock_barcode_action_main_menu')
    ...
    def _try_open_lot_location(self, barcode):
        ...
        if result:
            ...
            kanban_view_id = request.env.ref('mhma_stock_barcode.stock_quant_barcode_kanban_2').id
            return {
                'action': {
