...
export class ManualBarcodeScanner extends Component {
    ...
    async _onApply() {
        ...
        try {
            // onApply عندك يجب أن يرجع true/false أو Promise يتم انتظارها
            const ok = await this.props.onApply(this.state.barcode);
            if (ok) {
                this.props.close();
            }
        } catch (e) {
            // لا تترك promise بدون معالجة
            // (الرسالة الأفضل سيتم عرضها من model/notification)
        }
    }
